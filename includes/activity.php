<?php
/**
 * Adds meta box to achievement types for turning on/off Gravity Forms activity posts when a user earns an achievement
 *
 * @since 1.1.0
 */
...
//add_filter( 'cmb_meta_boxes', 'badgeos_gravityforms_custom_metaboxes' );
...
// No profile integration yet
//add_action( 'learndash_learner_profile', 'badgeos_gravityforms_learner_profile', 10, 1 );
...
function badgeos_gravityforms_step_link_title_to_object( $title = '', $step = null ) {
	...
	if ( 'gravityforms_trigger' == $requirements[ 'trigger_type' ] ) {
		...
		$object_triggers = array(
			'gform_after_submission' => 'gravityforms-entry'
			// 'badgeos_gravityforms_form_completed_specific' => 'gravityforms-entry'
		);
...
add_filter( 'badgeos_step_title_display', 'badgeos_gravityforms_step_link_title_to_object', 11, 2 );
